package com.example.book_quotes_hub.ui.theme


import androidx.compose.ui.graphics.Color

val purple_200 = Color(0xFFBB86FC)
val purple_500 = Color(0xFF6200EE)
val purple_700 = Color(0xFF3700B3)
val teal_200 = Color(0xFF03DAC5)
val teal_700 = Color(0xFF018786)
val black = Color(0xFF000000)
val white = Color(0xFFFFFFFF)

// Custom colors based on your usage in components
val text = Color(0xFF212121) // A common dark text color
val primary = purple_500 // Example: map primary from your colors.xml